...
/**
 * Archive Scanner class for scanning research and URL content archives
 */
  ...
  private async scanResearchArchive(dirPath: string, lastScanTime?: Date): Promise<Document[]> {
    ...
    for (const filePath of files) {
      try {
        ...
        console.error(`Error processing research file ${filePath}: ${err.message}`);
      }
    }
  ...
  private async scanUrlContentArchive(dirPath: string, lastScanTime?: Date): Promise<Document[]> {
    ...
    for (const filePath of files) {
      try {
        ...
        const err = error as Error;
        console.error(`Error processing URL content file ${filePath}: ${err.message}`);
      }
    }
  ...
  private extractResearchMetadata(filePath: string, content: string): Record<string, any> {
    ...
    if (frontmatterMatch) {
      try {
        ...
        const err = error as Error;
        console.warn(`Error parsing frontmatter in ${filePath}: ${err.message}`);
      }
    }
